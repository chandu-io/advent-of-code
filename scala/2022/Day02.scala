//val input = io.Source.fromFile("../../__input__/2022/day-02-input-sample.txt")
val input = io.Source.fromFile("../../__input__/2022/day-02-input.txt")
...
val outcomes: Map[(Shape, Shape), Outcome] = Map(
  (Rock, Rock) -> Draw,
  (Rock, Paper) -> Win,
  (Rock, Scissors) -> Loose,
  (Paper, Rock) -> Loose,
...
val myResponses: Map[(Shape, Outcome), Shape] = Map(
  (Rock, Draw) -> Rock,
  (Rock, Win) -> Paper,
  (Rock, Loose) -> Scissors,
  (Paper, Loose) -> Rock,
...
def pointsForStrategy1(abc: OpponentPlay, xyz: MyResponse): Int =
  outcomes(abc.shape, xyz.shape).points + xyz.shape.points

def pointsForStrategy2(abc: OpponentPlay, xyz: MyResponse): Int = xyz match
